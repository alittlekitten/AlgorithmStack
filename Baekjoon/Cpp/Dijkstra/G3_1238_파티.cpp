...
void dijkstra(int start) {
    for (int i = 0; i < 1001; ++i) {
        dist[i] = 100000; // 하나의 간선의 weight가 1 이상 100 이하이고, 최악의 경우 999개의 간선을 통해 도시가 연결될 수 있으므로 99900을 초과하는 값이 들어가야 하므로 100000을 대입
    }
    ...
    while (!pq.empty()) {
        ...
        if (dist[curr] < w) continue; // w가 기존 비용보다 크면 통과 (마지막 연산 최적화)
...
int main() {
    ...
    // 모든 vertex에 대해서 dijkstra를 돌리면서 최댓값 확인
    int maxValue = -1;
    for (int i = 1; i <= n; ++i) {
        if (i == x) continue;
        dijkstra(i);
        if (maxValue < dist[x] + tmp[i]) maxValue = dist[x] + tmp[i]; // i에서 x로 가는거 + x에서 i로 가는거 계산해서 최댓값 저장
    }

...
int main() {
    ...
    // 위 내용에 fibo(1) + fibo(3) + ... + fibo(n) = fibo(n+1) (n % 2 == 1) 이라는 것을 깨달으면 된다.
    // (엄밀하게 따지면 fibo(n+1) + fibo(1) - fibo(2)인데, fibo(1)과 fibo(2)는 모두 1이므로 서로 상쇄된다.
    // 직접 증명하자면 
    // fibo(n) = fibo(n-1) + fibo(n-2)에서 fibo(n-2)를 fibo(n-3)과 fibo(n-4)의 합으로 쪼개고, fibo(n-4)를 fibo(n-5)와 fibo(n-6)의 합으로 쪼개고..를 반복하면 마지막까지 가서 0이 더해져서 홀수번째 합이 나온다.
    ...
    else cout << fibo(n); // 짝수번째면 fibo(n) 출력
